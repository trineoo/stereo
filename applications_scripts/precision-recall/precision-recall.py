...
parser.add_argument("MINOVERLAP", type=float, help="set the minimum overlap for True Positive")
parser.add_argument("THRESHOLD", type=float, help="set the threshold for for YOLO")
...
MINOVERLAP = args.MINOVERLAP # default value (defined in the PASCAL VOC2012 challenge)
THRESHOLD = args.THRESHOLD # default value
print 'mAP running with min overlap (IoU) {0} and YOLO threshold {1}'.format(MINOVERLAP, THRESHOLD)
    ...
    for txt_file in dr_files_list:
        ...
        for line in lines:
            try:
                ...
                error_msg += " Received: " + line
                error(error_msg)
            if tmp_class_name == class_name and float(confidence)>=THRESHOLD:
                bbox = left + " " + top + " " + right + " " +bottom
...
"""
 Write total precision/recall
"""
with open(output_files_path + "/output.txt", 'a') as output_file:
    output_file.write("\n# Total precision/recall per class\n")
    for class_name in sorted(dr_classes):
        n_det = det_counter_per_class[class_name]
	gt_det = gt_counter_per_class[class_name]
	tp = count_true_positives[class_name]
	fp = n_det - count_true_positives[class_name]
        text = class_name + ": " + str(n_det)
	prec = float(tp)/(tp+fp)
	recall = float(tp)/gt_det
	print("Number objects of class: " + str(gt_det))
	print("Number of detected objects in class " + str(n_det))
	print("True positive: " + str(tp))
	print("False positive: " + str(fp))
	print("Precision: " + str(prec))
	print("Recall: " + str(recall))
        text += " (Precision:" + str(prec) + ""
